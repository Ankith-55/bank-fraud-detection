    ...
    def __init__(self):
        ...
        async def add_service(
            self, service_name:str, check_function:Callable[[],Awaitable[bool]], timeout:float=5.0, retry_delay:float=1.0,max_retries:int=3,depends_on:list[str] | None =None
                ...
                self._macx_retries[service_name]=max_retries
        ...
        async def check_celery(self) -> bool:
            try:
                inspect = celery_app.control.insepct()
        ...
        async def check_service_health(self,service_name:str,max_retries:int=3):
            ...
            if service_name not in self._chec_functions:
                raise ValueError(f"Unknown service: {service_name}")
            
            check_func=self._chechk_functions[service_name]
            ...
            async with self._lock:
                self._service[service_name] = ServiceStatus.UNHEALTHY
                logger.error(
        ...
        async def wait_for_services(self,timeout:float=30.0) -> bool:
            try:
                ...
                while(datetime.now() - start_time) < timedelta(seconds=timeout):
                    status=await self._check_all_service()
